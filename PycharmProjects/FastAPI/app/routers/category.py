...
async def update_category(db: Annotated[AsyncSession, Depends(get_db)],
                          ...
                          get_user: Annotated[dict, Depends(get_current_user)]):
    if get_user.get('is_admin'):
        category = await db.scalar(select(Category).where(Category.is_active == True, Category.slug == category_slug))
        if category is None:
            raise HTTPException(
...
async def delete_category(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    # Используем scalar() и first() для получения одной записи
    category = await db.scalar(
        select(Category)
        .where(Category.is_active == True, Category.slug == category_slug)
    )
    ...
    return {
        'status_code': status.HTTP_200_OK,
        'transaction': 'Category delete successful'
    }